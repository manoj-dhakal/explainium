var pdfjsLib = (function (exports) {
  ...
  class NodeCanvasFactory extends BaseCanvasFactory {
    _createCanvas(width, height) {
      const require = process.getBuiltinModule("module").createRequire("file:///Users/manojdhakal/dev/eli5/pdf.js/src/display/node_utils.js");
  ...
  const version = "4.9.112";
  const build = "8f08ca215";
...
console.log("Initializing PDF.js for extracting and selecting text...");
...
function fetchAndRenderPDF() {
  ...
  pdfjsLib.getDocument(url).promise
    .then((pdfDoc) => {
      ...
      renderPDFWithTextLayer(pdfDoc);
    })
    .catch((error) => console.error("Error loading PDF:", error));
}

function renderPDFWithTextLayer(pdfDoc) {
  const viewerContainer = document.createElement("div");
  viewerContainer.style.position = "relative";
  viewerContainer.style.overflow = "auto"; // Enable scrolling
  viewerContainer.style.width = "100%";
  viewerContainer.style.height = "100vh"; // Full viewport height
  ...
  for (let pageNum = 1; pageNum <= pdfDoc.numPages; pageNum++) {
    pdfDoc.getPage(pageNum).then((page) => {
      ...
      // Render the text layer on top of the page
      renderTextLayer(page, viewport, pageContainer);
    });
  }
}

function renderTextLayer(page, viewport, pageContainer) {
  page.getTextContent().then((textContent) => {
    const textLayerDiv = document.createElement("div");
textLayerDiv.className = "textLayer";

// Set the position to absolute to position it relative to its nearest positioned ancestor
textLayerDiv.style.position = "absolute";

// Center the element horizontally and vertically
textLayerDiv.style.top = "30%";
textLayerDiv.style.left = "100%";
textLayerDiv.style.transform = "translate(-50%, -50%)";

// Set the height and width to match the viewport dimensions
textLayerDiv.style.height = `${viewport.height}px`;
textLayerDiv.style.width = `${viewport.width}px`;

// Allow interactions with the text layer
textLayerDiv.style.pointerEvents = "auto";

// Keep the text layer above other elements
textLayerDiv.style.zIndex = "2";

// Ensure full visibility of the text layer
textLayerDiv.style.opacity = "1";

// Set a light semi-transparent background color
textLayerDiv.style.backgroundColor = "rgba(255, 255, 255, 1)";
    ...
    textContent.items.forEach((textItem) => {
      ...
      const [fontHeight, , , , x, y] = transform;
      span.style.transform = `matrix(${transform.slice(0, 4).join(",")})`;
      span.style.transformOrigin = "0 0";
      span.style.position = "absolute";
      span.style.left = `${x}px`;
      span.style.top = `${viewport.height - y}px`; // Flip y-axis
      span.style.fontSize = `${fontHeight}px`;
      span.style.whiteSpace = "pre";
      span.textContent = textItem.str;

      // Allow smooth text selection
      span.style.userSelect = "text";
      textLayerDiv.appendChild(span);
    });

    // Enable selection detection
    enableTextSelection(textLayerDiv);
  });
}
function enableTextSelection(textLayerDiv) {
  document.addEventListener("mouseup", () => {
    const selectedText = getCustomSelectedText(textLayerDiv);
    if (selectedText) {
      console.log("Selected text:", selectedText);

      // Send the selected text to the background script
      chrome.runtime.sendMessage({ type: "NEW_TEXT", text: selectedText }, (response) => {
...
function getCustomSelectedText(textLayerDiv) {
  const selection = window.getSelection();
